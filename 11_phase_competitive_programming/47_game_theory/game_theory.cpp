...
int grundy(int n, const std::vector<int>& moves, std::vector<int>& memo) {
    ...
    for (int mv : moves) {
        if (n >= mv)
            reach.insert(grundy(n - mv, moves, memo));
    }
...
int main() {
    ...
    std::cout << "Nim {3,4,5} first wins? " << (first_wins_nim({3, 4, 5}) ? "YES" : "NO")
              << " (expected YES)\n";
    std::cout << "Nim {1,2,3} first wins? " << (first_wins_nim({1, 2, 3}) ? "YES" : "NO")
              << " (expected NO)\n";
