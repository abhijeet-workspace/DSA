...
struct BipartiteKonig {
    ...
    BipartiteKonig(int nL_, int nR_) : nL(nL_), nR(nR_), adj(nL_), pairU(nL_, -1), pairV(nR_, -1) {}
    ...
    bool dfs(int u, std::vector<char>& used) {
        ...
        for (int v : adj[static_cast<size_t>(u)]) {
            if (pairV[static_cast<size_t>(v)] == -1 || dfs(pairV[static_cast<size_t>(v)], used)) {
    ...
    std::pair<int, std::vector<std::string>> minVertexCover() {
        ...
        while (!q.empty()) {
            ...
            q.pop();
            for (int v : adj[static_cast<size_t>(u)]) {
                if (pairU[static_cast<size_t>(u)] == v)
                    continue; // skip matching L→R
                if (inZ_R[static_cast<size_t>(v)])
                    continue;
        ...
        for (int u = 0; u < nL; ++u)
            if (!inZ_L[static_cast<size_t>(u)])
                cover.push_back("L" + std::to_string(u));
        for (int v = 0; v < nR; ++v)
            if (inZ_R[static_cast<size_t>(v)])
                cover.push_back("R" + std::to_string(v));
