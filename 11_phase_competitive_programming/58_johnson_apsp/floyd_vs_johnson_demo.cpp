...
std::vector<std::vector<long long>> floyd(int n, const std::vector<std::vector<long long>>& init) {
    auto d = init;
    for (int k = 0; k < n; ++k)
        for (int i = 0; i < n; ++i)
            for (int j = 0; j < n; ++j)
                if (d[static_cast<size_t>(i)][static_cast<size_t>(k)] < INF &&
                    ...
                    d[static_cast<size_t>(i)][static_cast<size_t>(j)] =
                        std::min(d[static_cast<size_t>(i)][static_cast<size_t>(j)],
                                 d[static_cast<size_t>(i)][static_cast<size_t>(k)] +
                                     d[static_cast<size_t>(k)][static_cast<size_t>(j)]);
    return d;
}

bool johnson(int n, const std::vector<Edge>& edges, std::vector<std::vector<long long>>& dist) {
    ...
    for (int s = 0; s < n; ++s) {
        ...
        for (int v = 0; v < n; ++v)
            if (d[static_cast<size_t>(v)] < INF)
                dist[static_cast<size_t>(s)][static_cast<size_t>(v)] = d[static_cast<size_t>(v)] -
...
static void printMat(const char* name, const std::vector<std::vector<long long>>& d) {
    std::cout << name << ":\n";
...
int main() {
    ...
    for (int i = 0; i < n; ++i)
        init[static_cast<size_t>(i)][static_cast<size_t>(i)] = 0;
    for (const Edge& e : edges)
