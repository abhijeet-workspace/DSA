...
bool dfs(std::vector<std::vector<char>>& board, int r, int c, const std::string& word, int i) {
    ...
    if (r < 0 || c < 0 || r >= static_cast<int>(board.size()) ||
        ...
        board[static_cast<size_t>(r)][static_cast<size_t>(c)] != word[static_cast<size_t>(i)]) {
        return false;
    }
    ...
    const bool found = dfs(board, r + 1, c, word, i + 1) || dfs(board, r - 1, c, word, i + 1) ||
...
int main() {
    ...
    std::cout << std::boolalpha << exist(board, "ABCCED") << " (expected true)\n";
    return 0;
}
