...
void backtrack(const std::string& s, int start, int segmentCount, std::string current,
    ...
    if (segmentCount >= 4 || start >= static_cast<int>(s.size()))
        return;

    for (int len = 1; len <= 3 && start + len <= static_cast<int>(s.size()); ++len) {
        const std::string seg = s.substr(static_cast<size_t>(start), static_cast<size_t>(len));
...
int main() {
    ...
    std::cout << "IPs for \"" << s << "\" (" << result.size() << " expected 2):\n";
