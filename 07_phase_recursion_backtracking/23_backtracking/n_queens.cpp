...
bool isValid(const std::vector<std::string>& board, int row, int col, int n) {
    for (int i = 0; i < row; ++i) { // same column
        if (board[static_cast<size_t>(i)][static_cast<size_t>(col)] == 'Q')
            return false;
    }
    for (int i = row - 1, j = col - 1; i >= 0 && j >= 0; --i, --j) { // left diag
        if (board[static_cast<size_t>(i)][static_cast<size_t>(j)] == 'Q')
            return false;
    }
...
std::vector<std::vector<std::string>> solveNQueens(int n) {
    std::vector<std::vector<std::string>> result;
    std::vector<std::string> board(static_cast<size_t>(n),
...
int main() {
    ...
    std::cout << "Found " << boards.size() << " solutions for " << n << "-Queens (expected 2)\n";
