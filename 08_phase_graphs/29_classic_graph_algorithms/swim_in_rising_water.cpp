...
int swimInWater(const std::vector<std::vector<int>>& grid) {
    ...
    std::vector<std::vector<char>> vis(static_cast<size_t>(n), std::vector<char>(static_cast<size_t>(n), 0));
    ...
    while (!pq.empty()) {
        ...
        for (int k = 0; k < 4; ++k) {
            ...
            if (nr < 0 || nc < 0 || nr >= n || nc >= n || vis[static_cast<size_t>(nr)][static_cast<size_t>(nc)])
                continue;
