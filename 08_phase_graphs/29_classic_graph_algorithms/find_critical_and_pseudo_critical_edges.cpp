...
struct DSU {
    ...
    int find(int x) { return p[static_cast<size_t>(x)] == x ? x : p[static_cast<size_t>(x)] = find(p[static_cast<size_t>(x)]); }
...
int mstWeight(int n, const std::vector<std::vector<int>>& edges, int forceIn, int forceOut) {
    ...
    for (int i = 0; i < static_cast<int>(edges.size()); ++i) {
        if (i == forceOut || i == forceIn) continue;
...
std::vector<std::vector<int>> findCriticalAndPseudoCriticalEdges(int n, std::vector<std::vector<int>> edges) {
    ...
    for (int i = 0; i < m; ++i) edges[static_cast<size_t>(i)].push_back(i); // original index
