...
int minimumEffortPath(const std::vector<std::vector<int>>& heights) {
    ...
    const int INF = 1e9;
    std::vector<std::vector<int>> dist(static_cast<size_t>(R),
                                       std::vector<int>(static_cast<size_t>(C), INF));
    ...
    while (!pq.empty()) {
        ...
        for (int k = 0; k < 4; ++k) {
            ...
            if (nr < 0 || nc < 0 || nr >= R || nc >= C)
                continue;
            int next =
                std::max(eff, std::abs(heights[static_cast<size_t>(nr)][static_cast<size_t>(nc)] -
