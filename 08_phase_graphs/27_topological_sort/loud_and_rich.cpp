...
int dfs(int u, const std::vector<std::vector<int>>& richerThan, const std::vector<int>& quiet,
    ...
    for (int v : richerThan[static_cast<size_t>(u)]) {
        ...
        if (quiet[static_cast<size_t>(cand)] <
            quiet[static_cast<size_t>(ans[static_cast<size_t>(u)])])
...
std::vector<int> loudAndRich(const std::vector<std::vector<int>>& richer,
    ...
    std::vector<std::vector<int>> richerThan(
        static_cast<size_t>(n)); // edge poor <- rich : rich points to?
    ...
    for (const auto& e : richer)
        richerPeople[static_cast<size_t>(e[1])].push_back(e[0]);
    std::vector<int> ans(static_cast<size_t>(n), -1);
    for (int i = 0; i < n; ++i)
        dfs(i, richerPeople, quiet, ans);
    return ans;
}
