...
void auditTransactions(std::vector<double>& prices) {
    ...
    std::cout << "   Tax due (15%): $" << calculatedTax << " | Capped Tax payment: $" << cappedTax
              << "\n";
}
...
void queryWarehouse(std::vector<int>& stockCodes, int queryCode) {
    ...
    std::cout << "Query Code " << queryCode << ": " << (exists ? "AVAILABLE" : "OUT OF STOCK")
              << "\n";

    if (exists) {
        auto lb =
            std::lower_bound(stockCodes.begin(), stockCodes.end(), queryCode); // first >= query
        std::cout << "   Indexed at location " << std::distance(stockCodes.begin(), lb) << "\n";
    }
}
...
int main() {
    ...
    std::cout << "Swapped: x = " << x << ", y = " << y << " | GCD(18, 24) = " << std::gcd(18, 24)
              << "\n";
