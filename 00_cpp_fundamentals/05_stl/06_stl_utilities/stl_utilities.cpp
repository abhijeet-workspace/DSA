...
void demonstratePairAndTuple() {
    ...
    std::cout << "Pair -> First (ID): " << student.first << ", Second (Name): " << student.second
              << std::endl;

    std::tuple<int, std::string, double> employee(201, "Amit", 75000.50);
    std::cout << "Tuple -> ID: " << std::get<0>(employee) << ", Name: " << std::get<1>(employee)
    ...
    std::cout << "Unpacked -> " << empName << " (ID: " << empId << ") earns $" << empSalary
              << std::endl;
}
...
void demonstrateAlgorithms() {
    ...
    if (it != vec.end()) {
        std::cout << "Element " << target << " found at index " << std::distance(vec.begin(), it)
                  << " using std::find." << std::endl;
    }

    int searchVal = 50;
    if (std::binary_search(vec.begin(), vec.end(), searchVal)) { // needs sorted
        std::cout << "Element " << searchVal
                  << " exists in the vector (verified by std::binary_search)." << std::endl;
    }

    auto lb = std::lower_bound(vec.begin(), vec.end(), 25); // first >= 25
    std::cout << "Lower bound for 25: " << *lb << " (at index " << std::distance(vec.begin(), lb)
              << ")" << std::endl;
}
