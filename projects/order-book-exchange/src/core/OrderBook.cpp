...
void OrderBook::addOrder(const Order& order) {
    ...
    if (order.type == OrderType::MARKET) {
        spdlog::warn(
            "MARKET orders not yet fully implemented, processing as LIMIT at 0/MAX for now");
    }
    ...
    if (order.side == Side::BUY) {
        ...
        spdlog::debug("Added BUY order {} for {} at {}", order.orderId, order.quantity,
                      order.price);
    } else {
        asks_[order.price].push_back(order);
        auto it = std::prev(asks_[order.price].end());
        orderMap_[order.orderId] = {it, order.price, Side::SELL};
        spdlog::debug("Added SELL order {} for {} at {}", order.orderId, order.quantity,
                      order.price);
    }
    matchOrders();
}
...
void OrderBook::matchOrders() {
    while (!bids_.empty() && !asks_.empty()) {
        ...
        if (bestBidIt->first >= bestAskIt->first) {
            ...
            spdlog::info("MATCH: {} units of {} at price {}", matchedQuantity, symbol_,
                         matchedPrice);
