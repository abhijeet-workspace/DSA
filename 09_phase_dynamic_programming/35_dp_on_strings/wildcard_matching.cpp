...
// Match entire string with '?' (one char) and '*' (any sequence) using DP / two-pointer greedy.
#include <iostream>
#include <string>
#include <vector>
...
int main() {
    ...
    std::cout << isMatch("aa", "*") << " (expected true)\n";
    std::cout << isMatch("cb", "?a") << " (expected false)\n";
    std::cout << isMatch("adceb", "*a*b") << " (expected true)\n";
    return 0;
}
