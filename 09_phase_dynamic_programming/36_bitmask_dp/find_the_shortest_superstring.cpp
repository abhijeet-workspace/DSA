...
std::string shortestSuperstring(std::vector<std::string>& words) {
    ...
    for (int i = 0; i < n; ++i)
        dp[1 << i][static_cast<size_t>(i)] = words[static_cast<size_t>(i)];
    for (int mask = 0; mask < N; ++mask) {
        for (int i = 0; i < n; ++i) {
            if (!(mask & (1 << i)) || dp[static_cast<size_t>(mask)][static_cast<size_t>(i)].empty())
                continue;
            for (int j = 0; j < n; ++j) {
                if (mask & (1 << j))
                    continue;
                ...
                std::string cand = dp[static_cast<size_t>(mask)][static_cast<size_t>(i)] +
                                   words[static_cast<size_t>(j)].substr(static_cast<size_t>(
                                       overlap[static_cast<size_t>(i)][static_cast<size_t>(j)]));
                auto& cur = dp[static_cast<size_t>(nmask)][static_cast<size_t>(j)];
                if (cur.empty() || cand.size() < cur.size())
                    cur = cand;
            }
        }
    }
    ...
    for (int i = 0; i < n; ++i) {
        ...
        if (!s.empty() && (ans.empty() || s.size() < ans.size()))
            ans = s;
    }
    return ans;
}
