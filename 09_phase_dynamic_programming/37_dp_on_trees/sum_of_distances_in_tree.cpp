    ...
    std::function<void(int, int)> dfs1 = [&](int u, int p) {
        for (int v : g[static_cast<size_t>(u)])
            if (v != p) {
                ...
                ans[static_cast<size_t>(u)] +=
                    ans[static_cast<size_t>(v)] + count[static_cast<size_t>(v)];
            }
    };
    std::function<void(int, int)> dfs2 = [&](int u, int p) {
        for (int v : g[static_cast<size_t>(u)])
            if (v != p) {
                ans[static_cast<size_t>(v)] = ans[static_cast<size_t>(u)] -
...
int main() {
    ...
    for (int x : sumOfDistancesInTree(6, edges))
        std::cout << x << ' ';
