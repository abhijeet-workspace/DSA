...
int minFallingPathSum(std::vector<std::vector<int>>& matrix) {
    ...
    for (int r = n - 2; r >= 0; --r) {
        for (int c = 0; c < n; ++c) {
            ...
            if (c > 0) best = std::min(best, matrix[static_cast<size_t>(r + 1)][static_cast<size_t>(c - 1)]);
            if (c + 1 < n) best = std::min(best, matrix[static_cast<size_t>(r + 1)][static_cast<size_t>(c + 1)]);
