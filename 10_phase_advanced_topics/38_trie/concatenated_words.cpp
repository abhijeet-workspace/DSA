...
std::vector<std::string> findAllConcatenatedWordsInADict(const std::vector<std::string>& words) {
    ...
    for (const auto& w : words) {
        ...
        std::function<bool(int, int)> dfs = [&](int i, int parts) -> bool {
            if (i == n) return parts >= 2;
            if (memo[static_cast<size_t>(i)] != -1 && parts > 0) return memo[static_cast<size_t>(i)];
            Node* cur = root;
            for (int j = i; j < n; ++j) {
                ...
                if (cur->end && dfs(j + 1, parts + 1)) {
                    if (parts > 0) memo[static_cast<size_t>(i)] = 1;
                    return true;
                }
            }
            if (parts > 0) memo[static_cast<size_t>(i)] = 0;
            return false;
        };
        if (dfs(0, 0)) ans.push_back(w);
    }
    return ans;
}

int main() {
    auto ans = findAllConcatenatedWordsInADict(
