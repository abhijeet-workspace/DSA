...
// Prefix sums + increasing deque of indices; negatives break plain sliding window.
#include <algorithm>
#include <climits>
#include <deque>
#include <iostream>
...
int shortestSubarray(const std::vector<int>& nums, int k) {
    ...
    int best = INT_MAX;
    for (int j = 0; j <= n; ++j) {
        while (!dq.empty() && prefix[static_cast<size_t>(j)] - prefix[static_cast<size_t>(dq.front())] >= k) {
            best = std::min(best, j - dq.front());
            dq.pop_front();
        }
        while (!dq.empty() && prefix[static_cast<size_t>(dq.back())] >= prefix[static_cast<size_t>(j)]) {
