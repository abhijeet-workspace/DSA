...
std::string numberToWords(int num) {
    ...
    std::function<std::string(int)> under1000 = [&](int n) -> std::string {
        ...
        if (n < 100) {
            ...
            if (n % 10) res += std::string(" ") + below20[n % 10];
            return res;
        }
        std::string res = std::string(below20[n / 100]) + " Hundred";
        if (n % 100) res += " " + under1000(n % 100);
        return res;
    };
    ...
    std::function<std::string(int)> helper = [&](int n) -> std::string {
        ...
        if (n < 1000000) {
            ...
            if (n % 1000) res += " " + under1000(n % 1000);
            return res;
        }
        if (n < 1000000000) {
            std::string res = helper(n / 1000000) + " Million";
            if (n % 1000000) res += " " + helper(n % 1000000);
            return res;
        }
        std::string res = under1000(n / 1000000000) + " Billion";
        if (n % 1000000000) res += " " + helper(n % 1000000000);
        return res;
    };
...
int main() {
    ...
    std::cout << numberToWords(1234567)
