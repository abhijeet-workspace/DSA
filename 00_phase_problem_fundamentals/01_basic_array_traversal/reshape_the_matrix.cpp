...
std::vector<std::vector<int>> matrixReshape(const std::vector<std::vector<int>>& mat, int r,
                                            int c) {
    ...
    std::vector<std::vector<int>> ans(static_cast<size_t>(r),
                                      std::vector<int>(static_cast<size_t>(c)));
    for (int i = 0; i < m; ++i) {
        for (int j = 0; j < n; ++j) {
            const int k = i * n + j;
            ans[static_cast<size_t>(k / c)][static_cast<size_t>(k % c)] =
