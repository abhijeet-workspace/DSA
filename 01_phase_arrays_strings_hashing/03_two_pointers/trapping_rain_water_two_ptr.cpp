...
int trap(const std::vector<int>& height) {
    ...
    while (L < R) {
        if (height[static_cast<size_t>(L)] < height[static_cast<size_t>(R)]) {
            if (height[static_cast<size_t>(L)] >= left_max)
                left_max = height[static_cast<size_t>(L)];
            else
                water += left_max - height[static_cast<size_t>(L)];
            ++L;
        } else {
            if (height[static_cast<size_t>(R)] >= right_max)
                right_max = height[static_cast<size_t>(R)];
            else
                water += right_max - height[static_cast<size_t>(R)];
            --R;
        }
    }
    return water;
